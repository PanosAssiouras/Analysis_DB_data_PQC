...
data_sorted_notnan = data_sorted[data_sorted['TOX_NM'].notna()&data_sorted['TOX_NM']!=0]
...
#data_merged = data_merged.drop_duplicates(subset=["FILE_NAME"], keep=False)
...
#----------------------------------------------------------------------------------------------#
#------- Filter outliers-----------------------------------------------------------------------#
z_scores = stats.zscore(data_merged['TOX_NM'])  # calculate z-scores of `df`
abs_z_scores = np.abs(z_scores)
filtered_entries = (abs_z_scores < 3)
new_data_merged = data_merged[filtered_entries]
#----------------------------------------------------------------------------------------------#
#------ reduce data_merged --------------------------------------------------------------------#
data_merged_reduced = new_data_merged[['batch_number', 'Type', 'TOX_NM', 'Location', 'KIND_OF_HM_SET_ID']]
data_merged_reduced.reset_index(drop=True, inplace=True)

#---------------------- calculate averages and stadard deviation--------------------------------#
data_merged_reduced_mean = data_merged_reduced.groupby(['batch_number', 'Type', 'Location']).mean()
data_merged_reduced_mean.reset_index(inplace=True)
data_merged_reduced_mean.rename(columns={'TOX_NM': 'Average'}, inplace=True)

data_merged_reduced_std = data_merged_reduced.groupby(['batch_number']).std()
data_merged_reduced_std.reset_index(inplace=True)
data_merged_reduced_std.rename(columns={'TOX_NM': 'Std.dev'}, inplace=True)
# data_merged_reducedstd.reset_index(drop=True,inplace=True)
...
# print(data_merged_reducedmean)
# print(data_merged_reducedstd)
# print(data_merged)

data_merged2.to_csv(r'MOS_TOX_NM_average_values.csv', index=False)
data_merged_reduced.to_csv(r'MOS_TOX_NM_all.csv', index=False)
