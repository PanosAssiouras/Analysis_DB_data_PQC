...
file_name_path = os.path.join(path, '../c9160.csv')
...
data_sorted_notnan = data_sorted_notnan.loc[data_sorted_notnan[parameter]!=0]
data_sorted_notnan = data_sorted_notnan.loc[(data_sorted_notnan[parameter]>=search_lower_limit)
                                            & (data_sorted_notnan[parameter]<=search_upper_limit)]
data_sorted_notnan.reset_index(drop=True, inplace=True)
data_sorted_notnan = data_sorted_notnan.drop('PART_BARCODE', axis=1)
...
metadata_reduced = metadata.iloc[:, 8:]
#metadata_reduced = metadata_reduced[metadata_reduced['KIND_OF_HM_STRUCT_ID'] == "GCD05"]
#metadata_reduced = metadata_reduced.drop_duplicates(subset=["FILE_NAME"], keep=False)
metadata_reduced.reset_index(drop=True, inplace=True)
#print(metadata_reduced)
metadata_reduced.to_csv(r'metadata_reduced.csv', index=False)
#----------------------------------------------------------------------------------------------#
#-------------- merge data with metadata and build file with all information-------------------#
data_merged = pd.merge(data_sorted_notnan, metadata_reduced, how='inner', on=['PART_NAME_LABEL'])
#data_merged = data_merged.drop('PART_BARCODE', axis=1)
#data_merged = data_merged.drop_duplicates(subset=["FILE_NAME"], keep='first')
...
print(data_merged_reduced_mean)
data_merged_reduced_std = data_merged_reduced.groupby(['batch_number', 'Type', 'Location'])[parameter].std().reset_index(name="std")
