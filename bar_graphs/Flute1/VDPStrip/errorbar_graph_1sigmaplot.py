...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        ax2.set_ylim(20.0, 10+round_up(ymax, n))
        ...
        ax2.set_ylabel("Sheet resistance [$\Omega$/sq]", fontsize=20)
        #####################################################################
        ...
        n, bins, patches = ax3.hist(data['Average'], range=(0, 20+round_up(ymax, n)), color='orange', bins=100, label="Histogram")
        x_min, x_max = ax3.get_xlim()
        x = np.linspace(x_min, x_max, 300)
        y = norm.pdf(x, mu, sigma)
        l = ax3.plot(x, y, 'r--', linewidth=2, label = "gauss fit")
        ax3.set_ylabel("Number of batches", fontsize=15)
        ax3.set_xlabel("Sheet resistance [$\Omega$/sq]", fontsize=12)
        # set parameters for tick labels ##############################################
