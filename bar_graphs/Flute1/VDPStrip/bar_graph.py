    ...
    if (pos!=-1):
        ...
        for index, row in data.iterrows():
            ...
            if row[1]=="Demokritos":
                ...
                fig2, ax2.bar(row[0], row[2], yerr=row[3], alpha=0.5, ecolor='black', capsize=10,
                              label='Measured at HEPHY', color='green')
            else:
                fig2, ax2.bar(row[0], row[2], yerr=row[3], alpha=0.5, ecolor='black', capsize=10,
                              label="Measured at Brown", color='red')
        ...
        plt.ylabel('Sheet Resistance [\u03A9/square]', fontsize=20)
        plt.xticks(rotation=45)

        # horizontal line indicating the threshold
        ax2.plot([-0.5, 17.5], [32.0, 32.0], "k--", color='gray')
        ax2.plot([-0.5, 17.5], [35.0, 35.0], "k--", color='gray')
