...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        values_of_2S_and_PSs = Flute1_results['VDPPoly_std']
        values_of_2S_and_PSs_new = [value for value in values_of_2S_and_PSs if math.isnan(value) == False]

        #values_of_PSp_batches = Flute1_results_PSP['VDPPoly_std']
        ...
        n, bins, patches = ax3.hist(values_of_2S_and_PSs, color='cyan', alpha=1.0, bins=bins_1, label=r"2S, PSS "
                                                                                                      "\n"
                                                                                                      r"and PSp wafers")
        xspace = np.linspace(1.5, 2.5, 10000)
        ...
        ax3.set_xlabel("Average value of sheet resistance [k$\Omega$/sq]", fontsize=15)
        ...
        print(xmin)
        print(x_max)
        ax2.set_title("VDP-Poly measurements", fontsize=22)
        ax2.axhline(y=mu1, xmin=xmin, xmax=x_max, label="Mean value $\mu$", color="black")
        ax2.axhline(y=2.2, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle='--')
        ax2.axhline(y=1.8, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle='--')
        #####################################################################
