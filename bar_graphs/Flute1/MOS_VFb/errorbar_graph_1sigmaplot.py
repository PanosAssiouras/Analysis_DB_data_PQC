...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        data_entries_2, bins_2 = np.histogram(data['Average'], bins=bins)
        binscenters = np.array([0.5 * (bins[i] + bins[i + 1]) for i in range(len(bins) - 1)])
        popt, pcov = curve_fit(fit_function, xdata=binscenters, ydata=data_entries_1, p0=[0.0, 1.0, -3.0, 0.3])
        print(popt)
        n, bins, patches = ax3.hist(data['Average'], range=(-6, 6), color='blue', bins=50, label="Histogram")
        #ax3.bar(binscenters, data_entries_1, width=bins[1] - bins[0], color='navy', label=r'Histogram entries')
        xspace = np.linspace(-8, 8, 100000)
        ax3.plot(xspace, fit_function(xspace, *popt), color='darkorange', linewidth=2.5, label=r'Fitted function')

        ax3.set_ylabel("Number of batches", fontsize=15)
        ax3.set_xlabel("Average value of flat-band voltage", fontsize=15)
        # set parameters for tick labels ##############################################
