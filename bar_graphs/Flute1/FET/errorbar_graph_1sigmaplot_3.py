...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        for index in data.index:
            ...
            if data['Type'][index] == "2-S":
               ...
               # values_of_2S_and_PSs.append(data['Average'][index])
                ...
                #values_of_2S_and_PSs.append(data['Average'][index])
                ax2.errorbar(names_of_PSs, values_of_PSs, yerr=errors_of_PSs, fmt='^', color='cyan', elinewidth=3,
        ...
        values_of_2S_and_PSs = Flute1_results['FET_Vth']
        values_of_2S_and_PSs_new = [value for value in values_of_2S_and_PSs if math.isnan(value) == False]

        values_of_PSp_batches = Flute1_results_PSP['FET_Vth']
        ...
        n, bins, patches = ax3.hist(values_of_PSp_batches_new, color='red', alpha=0.5, bins=bins_1, label="PSp wafers")
        xspace = np.linspace(0, 8, 10000)
        ...
        ax3.set_xlim(left=0, right=8)
        ax3.set_ylim(bottom=0.0)
        ax3.set_ylabel("Number of wafers", fontsize=15)
        ax3.set_xlabel("Threshold  voltage [V]", fontsize=15)
        ...
        ax2.set_ylabel("Threshold Voltage [V]", fontsize=25)
        #####################################################################
        #### tick labels #############################################################
