...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        Flute3_results = pd.read_csv("../Results_flute3.csv", sep=",", skiprows=0)
        Flute3_results_PSP = pd.read_csv("../Results_flute3_PSP.csv", sep=",", skiprows=0)
        ...
        values_of_2S_and_PSs = Flute3_results['VDPBulk_ResF1081']
        print(values_of_2S_and_PSs )
        #values_of_2S_and_PSs_new = [value*1E-3 for value in values_of_2S_and_PSs if math.isnan(value) == False]

        values_of_PSp_batches = Flute3_results_PSP['VDPBulk_ResF1081']
        #values_of_PSp_batches_new = [value*1E-3 for value in values_of_PSp_batches if math.isnan(value) == False]
        ...
        (mu1, sigma1) = norm.fit(values_of_2S_and_PSs)
        bins = np.linspace(0, 10.0, 60)
        data_entries_1, bins_1 = np.histogram(values_of_2S_and_PSs, bins=bins)
        binscenters = np.array([0.5 * (bins[i] + bins[i + 1]) for i in range(len(bins) - 1)])
        popt, pcov = curve_fit(fit_function, xdata=binscenters, ydata=data_entries_1, p0=[0.0, 0.0, mu1, sigma1])
        n, bins, patches = ax3.hist(values_of_2S_and_PSs, color='cyan', alpha=1.0, bins=bins_1, label="2S and PSs batches")
        xspace = np.linspace(0, 10.0, 10000)
        ...
        ax3.set_xlim(left=0, right=10.0)
        ax3.set_ylim(bottom=0.0)
        ax3.set_ylabel("Number of wafers", fontsize=15)
        ax3.set_xlabel("Bulk resistivity [k$\Omega$ cm]", fontsize=15)
        ...
        ax2.axhline(y=2.7, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle=':')
        #####################################################################
#        print("numb", 2*round_up(max(data['Average']), 0))
        ...
        ax2.set_ylabel("Bulk resistivity [k$\Omega$ cm]", fontsize=25)
        #####################################################################
        #### tick labels #############################################################
