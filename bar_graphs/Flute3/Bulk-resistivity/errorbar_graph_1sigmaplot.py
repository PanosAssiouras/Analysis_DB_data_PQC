...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        n, bins, patches = ax3.hist(data['Average'], range=(-round_up(ymax, n)-4.5, 4.5+round_up(ymax, n)), color='orange', bins=80, label="Histogram")
        x_min, x_max = ax3.get_xlim()
        x = np.linspace(x_min, x_max, 500)
        y = norm.pdf(x, mu, sigma)
        l = ax3.plot(x, y, 'r--', linewidth=2, label = "gauss fit")
        ax3.set_ylabel("Number of batches", fontsize=15)
        ax3.set_xlabel("Average value of bulk resistivity [k$\Omega$cm]", fontsize=15)
        # set parameters for tick labels ##############################################
