    ...
    if pos != -1:
        ...
        for index in data.index:
            ...
            if data['Type'][index] == "2S":
                ...
                values_of_2S = [data['Average'][index]]
                ...
                values_of_PSs=[data['Average'][index]]
                ...
                values_of_PSp = [data['Average'][index]]
        ...
        left, bottom, width, height = [0.20, 0.25, 0.25, 0.25]
        ...
        values_of_2S_and_PSs = Flute3_results['Diode_Vdep']
        values_of_2S_and_PSs_new = [value for value in values_of_2S_and_PSs if math.isnan(value) == False]

        #values_of_PSp_batches = Flute1_results_PSP['GCD S_0']
        ...
        bins = np.linspace(150, 350, 60)
        data_entries_1, bins_1 = np.histogram(values_of_2S_and_PSs_new, bins=bins)
        binscenters = np.array([0.5 * (bins[i] + bins[i + 1]) for i in range(len(bins) - 1)])
        popt, pcov = curve_fit(fit_function, xdata=binscenters, ydata=data_entries_1, p0=[0.0, 0.0, mu1, sigma1])
        n, bins, patches = ax3.hist(values_of_2S_and_PSs_new, color='cyan', alpha=1.0, bins=bins_1, label="2S and PSs batches")
        xspace = np.linspace(150, 350, 10000)
        ...
        ax3.set_xlim(left=150, right=350)
        ax3.set_ylim(bottom=0.0)
        ax3.set_ylabel("Number of batches", fontsize=15)
        ax3.set_xlabel("Average value of full depletion  voltage [V]", fontsize=15)
        ...
        ax2.axhline(y=350, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle=':')
        #####################################################################
#        print("numb", 2*round_up(max(data['Average']), 0))
        ...
        ax2.set_ylabel("Full depletion Voltage [V]", fontsize=25)
        #####################################################################
        #### tick labels #############################################################
        ...
        ax2.legend(by_label.values(), by_label.keys(), loc='lower right', prop={'size': 18})
        ##################################################################
