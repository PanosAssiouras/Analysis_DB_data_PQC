...
for file_name in file_names:
    ...
    if pos != -1:
        ...
        Flute1_results = pd.read_csv("../Results_flute2.csv", sep=",", skiprows=0)
        Flute1_results_PSP = pd.read_csv("../Results_flute2_PSP.csv", sep=",", skiprows=0)
        ...
        values_of_2S_and_PSs = Flute1_results['GCD S_0']
        ...
        xspace = np.linspace(0.0, 4.5, 10000)
        ...
        bins2 = np.linspace(0.0, 4.5, 80)
        data_entries_1, bins_1 = np.histogram(values_of_PSp_batches_new, bins=bins2)
        ...
        n, bins, patches = ax3.hist(values_of_PSp_batches, color='red', alpha=0.5, bins=bins_1, label="PSp wafers")
        #xspace = np.linspace(-5, -4, 10000)
        ...
        ax3.set_xlabel("Flatband  voltage [V]", fontsize=15)
        ...
        print(xmin)
        print(x_max)
        ax2.set_title("GCD S0", fontsize=22)
        ax2.axhline(y=mu1, xmin=xmin, xmax=x_max, label="Mean value $\mu$", color="black")
        #ax2.axhline(y=18, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle='--')
        #ax2.axhline(y=20, xmin=xmin, xmax=x_max, label="Spec limit", color="black", linestyle='--')
