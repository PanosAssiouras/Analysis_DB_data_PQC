...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        print(west_values)
        for tick in ind:
            fig2, ax2.bar(ind[tick], east_values[tick]*1e-3, alpha=0.7, width=0.4, ecolor='black', capsize=10, color='blue')
            fig2, ax2.bar(ind[tick]+width, west_values[tick]*1e-3, alpha=0.7, width=0.4, ecolor='black', capsize=10, color='cyan')

        colors = {'East halfmoon': 'blue', 'West halfmoon' : 'cyan'}
        labels = list(colors.keys())
        handles = [plt.Rectangle((0, 0), 1, 1, color=colors[label]) for label in labels]
        ax2.set_ylim(15.0, 20.0)
        ax2.set_title("Sheet resistance of p-stop for east and west halfmoons (VPX34351)", fontsize=20)
        plt.legend(handles, labels, prop={'size': 20})
        plt.xticks(fontsize=13)
        plt.yticks(fontsize=13)
        plt.xlabel('Wafer serial number',fontsize=20)
        plt.ylabel('Sheet resistance [k\u03A9/sq.]', fontsize=20)
