...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        width=0.4
        xtick_labels = convert([name.split("_")[1]+"_"+name.split("_")[2] for name in  data['Name']])
        print(xtick_labels)
        ...
        print(west_values)
        for tick in ind:
            fig2, ax2.bar(ind[tick], east_values[tick], alpha=0.7, width=0.4, ecolor='black', capsize=10, color='blue')
            fig2, ax2.bar(ind[tick]+width, west_values[tick], alpha=0.7, width=0.4, ecolor='black', capsize=10, color='cyan')

        colors = {'East halfmoon': 'blue', 'West halfmoon' : 'cyan'}
        labels = list(colors.keys())
        handles = [plt.Rectangle((0, 0), 1, 1, color=colors[label]) for label in labels]
        ax2.set_ylim(2.0, 6.0)
        ax2.set_title("Threshold voltage for east and west halfmoons (VPX34351)", fontsize=20)
        plt.legend(handles, labels, prop={'size': 20})
        plt.xticks(fontsize=13)
        plt.yticks(fontsize=13)
        plt.xlabel('Wafer serial number',fontsize=20)
        ...
        ax2.text(8.0, 4.8, 'Left flute set', color='red', ha='center', fontsize=25)
        ax2.text(3.0, 4.8, 'Right flute set', color='red', ha='center', fontsize=25)
